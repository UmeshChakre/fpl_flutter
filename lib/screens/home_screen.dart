                  ...
                  Row(
                    children: [
                      Expanded(
                          flex: 6,
                          child: Center(
                              child: Text("Players", style: subtitleStyle))),
                      Expanded(
                          flex: 1,
                          child: Text(
                            "Gw Point",
                      ...
                      data: (data1) {
                        return leagueData.when(
                            ...
                            data: (data) {
                              ...
                              final List<Result> sortGwPoint =
                                  List.from(totalPoint);
                              sortGwPoint.sort((a, b) =>
                                  b.eventTotal.compareTo(a.eventTotal));
                              return ListView.builder(
                                shrinkWrap: true,
                                itemCount: sortData == 'gwpoints'
                                    ? sortGwPoint.length
                                    : totalPoint.length,
                                physics: const NeverScrollableScrollPhysics(),
                                itemBuilder: (context, index) {
                                  final picksdata = ref.watch(
                                      teamPicksDataProvider(Tuple2(
                                          sortData == 'gwpoints'
                                              ? sortGwPoint[index].entry
                                              : totalPoint[index].entry,
                                          data1.events!
                                              .where((element) =>
                                  ...
                                  return picksdata.when(
                                      data: (seconddata) {
                                        return ListTile(
                                          ...
                                          subtitle: Text(
                                            sortData == 'gwpoints'
                                                ? sortGwPoint[index].playerName
                                                : totalPoint[index].playerName,
                                            style: subtitleStyle,
                                          ),
                                          trailing: Row(
  ...
  _buildAppbar(WidgetRef ref, BuildContext context) {
    ...
    return CustomAppbar(
        title: leagueData.when(
          data: (data) => Text(
            data.league.name.toString(),
            style:
                titleStyle.copyWith(color: dark, fontWeight: FontWeight.bold),
          ),
        ...
        children: [
          Row(
            ...
            children: [
              const Spacer(),
              SimpleDialogOption(
                child: Text("Yes", style: subtitleStyle),
                onPressed: () {
                  exit(0);
                },
              ),
              SimpleDialogOption(
                child: Text("No", style: subtitleStyle),
                onPressed: () {
                  Navigator.pop(context);
                },
              ),
